...
public class DaoQueryHandler4Oracle implements ServiceHandler<DaoQuery> {
	private final static Logger log = Logger.getLogger(DaoQueryHandler4Oracle.class);
	...
	public Data handle(DaoQuery annotation, Data request, ServiceHandlerContext context) throws Exception {
		...
		try {
			...
			this.fillServiceField(annotation, viewMetadata, rows);
			this.resetFields(viewMetadata, rows);
			//
			if (annotation.single()) {
				if(rows.isEmpty()) 
					result = new Data(Codes.CODE, Codes.OBJECT_NOT_EXISTS);
				else
					result = new Data((Map<String, Object>) (rows.get(0))).put(Codes.CODE, Codes.SUCCESS);
			} else {
				final int total;
				...
				result = new Data(Codes.CODE, Codes.SUCCESS, annotation.resultsets(), rows, Codes.TOTAL_ROWS, total).merge(request, false);
...
//				result.put("pageNums", pages);
			...
			result = new Data(Codes.CODE, Codes.FAIL, Codes.MSG, e.getMessage());
		}
		return result;
	}

	private void resetFields(Metadata viewMetadata, List<Map<String, Object>> rows) throws Exception {
		final List<Field> aliasFields = viewMetadata.getFields();
		for(Map<String, Object> row: rows) {
			for(Field f: aliasFields) {
				final String orifn = f.getName();
				final String fn = StringUtils.isEmpty(f.getAlias()) ?f.getName() : f.getAlias();
				row.put(fn, row.remove(orifn));
			}
		}
	}
	
	private void fillServiceField(DaoQuery annotation, Metadata viewMetadata, List<Map<String, Object>> rows) throws Exception {
		...
		for(Map<String, Object> row: rows) {
			for(Field f: serviceFields) {
				...
				final String valueField = (String)f.get("valueField");
