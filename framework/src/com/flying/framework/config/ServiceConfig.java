...
public class ServiceConfig extends BaseConfig{
	...
	public ServiceConfig(Service service, Class<?> serviceClass) {
		...
		for(com.flying.framework.annotation.event.Event c: service.events()) 
			this.eventConfigs.add(new EventConfig(c));
	}
	...
	private static Map<String, String> getConfigsFromService(Service service) {
		...
		for(com.flying.framework.annotation.service.ServiceConfig sc: service.configs()) {
	...
	public void merge(ServiceConfig sc) {
		this.eventConfigs.addAll(sc.eventConfigs);
		this.configs.merge(sc.configs, false);
	}
}
