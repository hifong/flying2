...
import com.flying.framework.exception.ServiceAlreadyDefinedException;
...
public class ModuleConfig extends Data {
	...
	public ModuleConfig(String moduleHome) {
		this(moduleHome, new File(moduleHome + File.separator + "config"+File.separator+"module.xml"));
	}

	public ModuleConfig(final String moduleHome, File moduleFile) {
	...
	private void loadProperties() {
		...
		if(super.contains("serviceConfigs")) {
			Map<String, ServiceConfig> scs = (Map<String, ServiceConfig>) super.get("serviceConfigs");
			for(Entry<String, ServiceConfig> sc: scs.entrySet()) {
				this.addServiceConfig(sc.getKey(), sc.getValue());
			}
		}
	...
	static Map<String, Object> loadModuleConfigFromFile(File file) {
		...
		try {
			...
			throw new AppException(e1, "load application config from " + file.getAbsolutePath() +" error!");
	...
	private void processImports() {
		...
		if (imports != null) {
			...
			for (String m : importModules) {
				File f = new File(m);
				ModuleConfig importModule = new ModuleConfig(this.moduleHome, f);
	...
	public String getTemplatePath() {
		return getPath(new String[] {"template"});
	}

	public String getLibPath() {
		return getPath(new String[] {"WEB-INF", "lib"});
	}

	public String getClassesPath() {
		return getPath(new String[] {"WEB-INF", "classes"});
	}
	
	public String getPath(String[] subpaths) {
		if(subpaths == null) return this.moduleHome;
		
		StringBuilder sb = new StringBuilder(this.moduleHome + File.separator);
		for(String subpath: subpaths) {
			sb.append(subpath + File.separator);
		}
		return  sb.toString();
	}
	...
	public boolean existsServiceConfig(String key) {
		return this.serviceConfigs.containsKey(key);
	}
	
	public void addServiceConfig(String serviceId, ServiceConfig serviceConfig) {
		final String serviceClassName = serviceConfig.getTarget();
		if(this.serviceConfigs.containsKey(serviceId)) {
			final ServiceConfig existsServiceConfig = this.getServiceConfig(serviceId);
			final String existsServiceClassName = existsServiceConfig.getTarget();
			
			if(StringUtils.equals(serviceClassName, existsServiceClassName)) 
				existsServiceConfig.merge(serviceConfig);
			else
				throw new ServiceAlreadyDefinedException(this.getId(), serviceId, serviceClassName, existsServiceClassName);
		} else {
			this.serviceConfigs.put(serviceId, serviceConfig);
			this.serviceConfigs.put(serviceClassName, serviceConfig);
		}
	}
