...
public class ModuleEntryFilter  implements Filter {
	...
	public void doFilter(ServletRequest sreq, ServletResponse sresp,
		...
		if(path.indexOf(".") > 0) {
			...
			if(Application.getInstance() != null && Application.getInstance().getModules() != null)
			...
			Principal principal = req.getSession()== null?null:(Principal)req.getSession().getAttribute(Principal.PRINCIPAL);
			...
			if(module != null) {
				...
				DataUtils.convert(module, req);
				ServiceContext.createLocalInvokingContext(module, principal, req, resp);
			}
		}
		//
		try {
	...
	private void processRequestInChain(LocalModule module, String requestType, HttpServletRequest req, HttpServletResponse resp, String[] pathItems)
		...
		try {
			...
			if(e instanceof com.flying.framework.exception.SecurityException) {
				resp.sendError(403);
