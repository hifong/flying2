{if $metadata.conditions }
 	...
 	<form name="{$id}_query_form" id="{$id}_query_form">
    	...
    	<div class="fitem" style="display:inline-block; width:46%; padding:5px">
            ...
            <input field="{$id}_query" id="{$field.name}" name="{$field.name}" style="cursor:hand;width:60%" class="easyui-combobox" {if $field.required eq 'true'}required{/if}
		        data-options="   
            ...
            <input field="{$id}_query" id="{$field.name}" name="{$field.name}" style="cursor:hand;width:60%" class="easyui-combotree" {if $field.required eq 'true'}required{/if}
		        data-options="   
			        valueField: '{$field.valueField}',   
			        textField: '{$field.textField}', 
			        value:'{$field.value | default:''}',
    	...
    	<div class="fitem" style="padding:5px; text-align:center;">
    		...
    		<input type="button" value="清除条件" onclick="queryClear('{$id}')">
    	</div>
    </form>
    </div>
 {/if}
